...
# Edit Markdown programmatically
markdown-edit README.md replace "Old Section" --with-string "## Old Section\nNew content"
...
markdown-doc mv old.md new.md
```

**New here?** Start with [`markdown-extract` Usage](#usage) below, then explore [`markdown-edit`](#companion-cli-markdown-edit) and [`markdown-doc`](#companion-cli-markdown-doc) for more advanced workflows.
...
The `markdown-doc` toolkit provides repository-wide documentation management with commands for cataloging, linting, validation, TOC management, and safe file operations.

**Quick Navigation:**
- [`catalog`](#catalog---generate-documentation-index) – Generate documentation indexes
- [`lint`](#lint---quality-checks) – Validate links and documentation quality
- [`validate`](#validate---schema-enforcement) – Enforce structural templates
- [`toc`](#toc---table-of-contents-synchronization) – Manage table of contents blocks
- [`mv`](#mv---safe-markdown-renames) – Rename/move files with automatic link updates
- [`refs`](#refs---reference-search) – Find all references to files or anchors
...
> ⚠️ `DOC_CATALOG.md` is auto-generated by `markdown-doc catalog`. Regenerate it; don’t hand-edit.
...
#### `refs` - Reference Discovery

Search the workspace for inbound references to Markdown files or anchors.

```console
# List all references to docs/guide.md (any anchor)
$ markdown-doc refs docs/guide.md

# Target a specific anchor
$ markdown-doc refs docs/guide.md#setup

# Anchor-only search across the repo
$ markdown-doc refs setup --anchor-only

# Limit search scope and emit JSON
$ markdown-doc refs docs/guide.md#setup --path docs/ --format json
```

Behaviour notes:
- Patterns accept relative paths (optionally with glob syntax) plus an optional `#anchor` suffix.
- Use `--anchor-only` to match by slug regardless of the target file.
- Results respect `.markdown-doc-ignore` and can be scoped via `--path` or `--staged`.
- Plain output shows `path:line -> target | snippet`; JSON mirrors `{"query": ..., "matches": [...]}` for automation.

#### Link Graph & Rewrite Utilities

The `markdown-doc mv` and `refs` commands are built on a shared **link graph engine** that tracks all relationships between Markdown files. This foundation enables safe refactoring operations by understanding the complete web of links, anchors, and references in your documentation.

**For library users:** Phase 3 introduces a reusable refactor engine in `markdown-doc-ops::refactor` that higher-level commands (`markdown-doc mv`, `markdown-doc refs`, future rename workflows) can leverage.
...
// plan.edits contains TextEdit for each affected file
// plan.moves contains FileMove entries with output paths
```

This API supports batch operations, multi-hop reference chains, and anchor-aware rewrites while keeping complexity hidden from CLI consumers.
### Configuration
...
Looking for a direct Python interface? Install the bundled PyO3 bindings via
`maturin develop --manifest-path crates/markdown_extract_py/Cargo.toml` (and the
matching commands for `markdown_edit_py` and `markdown_doc_py`). The packages
live under `python/` and are documented in `python/README.md`.
...
```bash
# Safe file renames with automatic link updates
markdown-doc mv old-name.md new-name.md --dry-run  # Preview first
markdown-doc mv old-name.md new-name.md            # Apply changes

# JSON output for automation
result=$(markdown-doc mv src.md dest.md --json --quiet)
files_updated=$(echo "$result" | jq '.files_updated')
echo "Updated $files_updated files"

# Find all references to a file or anchor
markdown-doc refs docs/guide.md --format json | \
  jq -r '.matches[] | "\(.path):\(.line)"'

# Check impact before moving
refs=$(markdown-doc refs old.md --format json)
ref_count=$(echo "$refs" | jq '.matches | length')
echo "This file is referenced $ref_count times"
```
